...
module.exports = {
    name: 'compquiz',
    description: 'Take a quiz with friends',
    category: "Chem",
    usage: "!compquiz",
    aliases: ["comptrivia", "comp", "cq"],
    execute: async (client, message, args, Client) => {
        if (!quizQuestions) return message.channel.send("An error occurred whilst fetching a quiz for you, please report this to Pbknowall#7857").catch(err => { console.log(err); return })
        ...
        try {
            ...
            collector.on("end", () => {
                if (!winner) return message.channel.send(Client.err("❌ Time's Up!").setDescription(`Looks like nobody got the answer in time! The correct answer was **${letter}** - **${quiz.correct}**`)).catch(err => { console.log(err); return })
            })
        } catch (err) {
            console.log(err)
            return message.channel.send(Client.err("❌ Time's Up!").setDescription(`Looks like nobody got the answer in time! The correct answer was **${letter}** - **${quiz.correct}**`)).catch(err => { console.log(err); return })
        }
    }
}